      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {processSteps.map((step, i) => (
            ...
            >
              <div className="text-[11px] font-bold text-[#1E3A8A] bg-[#F0F4FF] px-2 py-0.5 rounded-full inline-block mb-3 tracking-widest">
                {step.step}
              </div>
            ...
            {processSteps.map((step, i) => (
              <div key={i} className="relative flex items-start gap-4 pl-12">
                <div className="absolute left-0 w-10 h-10 rounded-full bg-white border-2 border-[#1E3A8A] flex items-center justify-center flex-shrink-0">
                  <span className="text-[10px] font-bold text-[#1E3A8A]">{step.step}</span>
                </div>
